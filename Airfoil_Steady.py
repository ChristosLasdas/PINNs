...
airfoil  = dde.geometry.Polygon(boundaryNACA4D(4, 4, 12, 0.2, 250, 0.20, 0.35))
geom     = dde.geometry.CSGDifference(farfield, airfoil)

inner_rec  = dde.geometry.Rectangle([0.15, 0.28], [xmax, 0.42])
...
inner_points = inner_dom.random_points(40_000)
outer_points = outer_dom.random_points(60_000)

farfield_points = farfield.random_boundary_points(1280)
airfoil_points  = boundaryNACA4D(4, 4, 12, 0.2, 125, 0.20, 0.35)
...
def fun_u_farfield(x, y, _):
    return dde.grad.jacobian(y, x, i = 0, j = 1) - umax
...
losshistory, train_state = model.train(epochs = 5_000, display_every = 100, model_save_path = './')
...
airfoil_plot = boundaryNACA4D(4, 4, 12, 0.2, 150, offset_x = 0.20, offset_y = 0.35)
...
plt.colorbar(cnt1, label="Pressure (p)")
plt.title("Streamlines and Contour Plot of Pressure (p)")
plt.xlabel("x")
plt.ylabel("y")
plt.savefig('NACA4412NS0.png')
plt.show()
...
plt.axis('equal')
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
plt.colorbar(cnt2, label="Velocity (u)")
plt.title("Streamlines and Contour Plot of Velocity (u)")
plt.xlabel("x")
plt.ylabel("y")
plt.savefig('NACA4412NS1.png')
plt.show()
...
plt.axis('equal')
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
plt.colorbar(cnt3, label="Velocity (v)")
plt.title("Streamlines and Contour Plot of Velocity (v)")
plt.xlabel("x")
plt.ylabel("y")
plt.savefig('NACA4412NS2.png')
plt.show()
...
clev = np.arange(vel_mag.min(), vel_mag.max(), 0.001)
cnt4 = ax4.contourf(x, y, vel_mag, clev, cmap = plt.cm.coolwarm, extend='both')
plt.axis('equal')
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
plt.colorbar(cnt4, label="Velocity Magnitude")
plt.title("Velocity Magnitude")
plt.xlabel("x")
plt.ylabel("y")
plt.savefig('NACA4412NS3.png')
plt.show()